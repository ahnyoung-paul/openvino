    ...
    for (auto& kvp : configMap) {
        ...
        if (hints.end() != std::find(hints.begin(), hints.end(), key)) {
            ...
            if (val.find(GPUConfigParams::GPU_PLUGIN_PRIORITY_HIGH) != std::string::npos) {
                queuePriority = cldnn::priority_mode_types::high;
                stream_exec_config._threadPreferredCoreType = IStreamsExecutor::Config::BIG;
            ...
            if (getAvailableCoresTypes().size() > 1) {
                if (stream_exec_config._threadPreferredCoreType == IStreamsExecutor::Config::BIG
                    || stream_exec_config._threadPreferredCoreType == IStreamsExecutor::Config::LITTLE) {
                        stream_exec_config._threadsPerStream = std::min(stream_exec_config._threadsPerStream,
                                                        getNumberOfCores(stream_exec_config._threadPreferredCoreType));
                    }
            } else {
                stream_exec_config._threadPreferredCoreType = IStreamsExecutor::Config::ANY;
                stream_exec_config._threadsPerStream = std::min(stream_exec_config._threadsPerStream,
...
void Config::adjustKeyMapValues() {
    ...
    {
        if (queuePriority == cldnn::priority_mode_types::high && stream_exec_config._threadPreferredCoreType == IStreamsExecutor::Config::BIG) {
            key_config_map[GPUConfigParams::KEY_GPU_PLUGIN_PRIORITY] = GPUConfigParams::GPU_PLUGIN_PRIORITY_HIGH;
        } else if (queuePriority == cldnn::priority_mode_types::low && stream_exec_config._threadPreferredCoreType == IStreamsExecutor::Config::LITTLE) {
    ...
    key_config_map[GPUConfigParams::KEY_GPU_MAX_NUM_THREADS] = std::to_string(stream_exec_config._threadsPerStream);
