 ...
 *      it has 4 types of levels: High, Med, Low, and Disabled. the default is Disabled
 ...
 *
 * There are two types of setting you can choose from: Plugin level setting and Queue/Host Task level setting.
 ...
 *
 * The default value of GPU_PLUGIN_PRIORITY is "GPI_QUEUE_PRIORITY_DISABLED|GPU_HOST_TASK_PRIORITY_ANY".
 * The detailed option values are as follows:
 * Plugin priority
 * • GPUConfigParams::GPU_PLUGIN_PRIORITY_HIGH  - GPU_QUEUE_PRIORITY_HIGH|GPU_HOST_TASK_PRIORITY_HIGH
 * • GPUConfigParams::GPU_PLUGIN_PRIORITY_LOW   - GPU_QUEUE_PRIORITY_LOW|GPU_HOST_TASK_PRIORITY_LOW
 ...
 * • GPUConfigParams::GPI_QUEUE_PRIORITY_DISABLED   - Not set queue priority property in cl_queue_properties
 * Host task priority
...
DECLARE_GPU_CONFIG_VALUE(PLUGIN_PRIORITY_HIGH);
DECLARE_GPU_CONFIG_VALUE(PLUGIN_PRIORITY_LOW);
...
DECLARE_GPU_CONFIG_VALUE(QUEUE_PRIORITY_DISABLED);
