...
void kernels_cache::build_all() {
...
#if (CLDNN_THREADING == CLDNN_THREADING_TBB)
        auto kernel_cache_config = _engine.configuration().stream_exec_config;
        kernel_cache_config._name = "CLDNNPlugin executor for build kernels cache on load network";

        _task_executor = std::unique_ptr<CPUStreamsExecutor>(new CPUStreamsExecutor(kernel_cache_config));
#elif(CLDNN_THREADING == CLDNN_THREADING_THREADPOOL)
    ...
    auto build_batch_func = [this, &_build_engine, &batches, &exception] {
        try {
    ...
    tasks.push_back(build_batch_func);
