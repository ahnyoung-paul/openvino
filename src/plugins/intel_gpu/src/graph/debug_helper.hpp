...
private:
    ...
    std::string get_file_prefix() {
        ...
        std::stringstream ss;
        ss << std::fixed << std::setw(6) << std::setfill('0') << m_inst.get_node().get_unique_id();

        return "program" + std::to_string(prog_id) + "_network" + std::to_string(net_id)
                    + "_" + get_iteration_prefix() + "_" + ss.str() + "_" + m_inst.id();
    }
