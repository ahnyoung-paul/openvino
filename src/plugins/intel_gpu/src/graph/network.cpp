...
std::map<primitive_id, network_output> network::execute(const std::vector<event::ptr>& dependencies) {
    ...
    // if (func_iter == 5) { // dump graph when func_iter is 5
    //     std::string dump_file_name = "debug_5_execution_result";
    //     _program->dump_program(dump_file_name.c_str(), true);
    // }
    return result;
}
...
#if 0
    if (node.id() == "constant:1563")
        std::cout << "[DEBUG] All constant nodes use usm device memory" << std::endl;
    if (node.is_constant())
        transfer_memory_to_device(inst, node);
#else
    if (node.id() == "constant:1563")
        std::cout << "[DEBUG] Some node which has reshape node as user node uses usm host memory" << std::endl;
    if (node.is_constant()) {
        auto users = node.get_users();
        if (users.size() == 1) {
            for (const auto& usr : users) {
                if (usr->is_type<reshape>() && usr->is_dynamic()) {
                    {
                        auto& inst_mem = inst->output_memory();
                        auto alloc_type = inst_mem.get_allocation_type();

                        if (alloc_type == allocation_type::usm_device) {
                            // Allocate and transfer memory
                            auto host_mem = inst_mem.get_engine()->allocate_memory(inst_mem.get_layout(), allocation_type::usm_host, false);
                            host_mem->copy_from(get_stream(), inst_mem);
                            _memory_pool->release_memory(&inst_mem, node.id(), get_id());
                            inst->set_output_memory(host_mem);
                        }
                    }
                } else {
                    transfer_memory_to_device(inst, node);
