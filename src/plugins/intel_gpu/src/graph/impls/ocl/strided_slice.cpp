...
public:
    static primitive_impl* create(const strided_slice_node& arg) {
        ...
        if (arg.is_dynamic()) {  // Dynamic shape
        ...
        if (!arg.is_dynamic()) {  // Static shape
            begin_mask.assign(begin_mask_.begin(), begin_mask_.end());
            end_mask.assign(end_mask_.begin(), end_mask_.end());
            new_axis_mask.assign(new_axis_mask_.begin(), new_axis_mask_.end());
            shrink_axis_mask.assign(shrink_axis_mask_.begin(), shrink_axis_mask_.end());
        }
