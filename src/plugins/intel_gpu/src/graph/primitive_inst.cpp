...
event::ptr primitive_inst::realloc_if_needed() {
    ...
    if (_node->get_users().size() == 1 && _node->get_users().front()->is_type<concatenation>()) {
        ...
        if (concat_inst->can_be_optimized()) {
            ...
            // reinter pret output memmory with dynamic padding
            this->_outputs[0] = _network.get_engine().reinterpret_buffer(*concat_inst->_outputs[0], get_output_layout());
    ...
    if (_node->get_users().size() == 1 && _node->get_users().front()->is_type<reshape>() && _node->get_users().front()->can_be_optimized()) {
        auto reshape_node = _node->get_users().front();
        if (reshape_node->get_users().size() == 1 && reshape_node->get_users().front()->is_type<concatenation>()) {
            auto reshape_inst = _network.get_primitive(_node->get_users().front()->id());
            auto concat_inst = _network.get_primitive(reshape_node->get_users().front()->id());
            if (concat_inst->can_be_optimized()) {
                if (!concat_inst->allocation_done_by_other) {
                    concat_inst->realloc_if_needed();
                    concat_inst->allocation_done_by_other = true;
                ...
                reshape_inst->_outputs[0] = _network.get_engine().reinterpret_buffer(*concat_inst->_outputs[0],
                                                                                        reshape_inst->get_output_layout());
                this->_outputs[0] = _network.get_engine().reinterpret_buffer(*concat_inst->_outputs[0], this->get_output_layout());
                GPU_DEBUG_TRACE_DETAIL << id() << ": use concat user's memory " << this->_outputs[0]->buffer_ptr() << std::endl;
                return ev;
            }
        }
...
void primitive_inst::do_runtime_in_place_concat() {
    ...
    if (!concat_inst->get_node().is_type<concatenation>() || !concat_inst->get_node().can_be_optimized()) {
        ...
        // change concat_inst to the user of optimized reshape.
