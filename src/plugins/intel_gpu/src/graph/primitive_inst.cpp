...
primitive_inst::primitive_inst(network& network, program_node const& node, bool allocate_memory)
    ...
    if (!_mem_allocated && (node.is_dynamic() && _impl_params->output_layouts.size() > 1)) {
        ...
        std::cout << "primitive_inst[" << id() << "] : allocate_memory " << allocate_memory
                    << ", " << _impl_params->output_layouts.size()
                    << ", node.is_dynamic(): " << node.is_dynamic() << std::endl;
    }
