    ...
    ov::matcher_pass_callback callback = [OV_CAPTURE_CPY_AND_THIS](ov::pass::pattern::Matcher& m) {
        ...
        for (auto& input : matmul_node->inputs()) {
            ...
            if (src_convert) {
                ...
                replace_convert_dt = true;
            } else {
                insert_converts_before_if_needed(matmul_node, desired_et, input_idx);
            }
        }

        // Step 2: Insert restore converts only if RoPE expects non-f32 precision.
        // When replace_convert_dt is true (e.g. Gemma4), the existing Convert was replaced in-place,
        // and RoPE can consume f32 cos/sin directly — no restore convert needed.
        // For other models, restore converts are inserted conservatively; they could potentially be
        // removed once verified that RoPE handles f32 inputs without issues across all models.
        if (!replace_convert_dt) {
            size_t output_idx = 0;
...
bool IncreasePositionIdsPrecision::run_on_model(const std::shared_ptr<ov::Model>& model) {
    ...
    symbolic_ctx_manager->register_pass<IncreasePositionIdsPrecisionForGPTOSS>();
    {
        for (const auto& n : model->get_ordered_ops()) {
            if (n->get_friendly_name().find("Sin") != std::string::npos) {
                const std::string dump_file = dump_folder + "\\before_increase_precision.svg";
                ov::pass::VisualizeTree(dump_file).run_on_model(model);
                std::cout << "!!!!!!!!!!!!!!! Graph dumped to: " << dump_file << std::endl;
                break;
            }
        }
    }
    auto ret = symbolic_optimizations.run_on_model(model);
    {
        for (const auto& n : model->get_ordered_ops()) {
            if (n->get_friendly_name().find("Sin") != std::string::npos) {
                const std::string dump_file = dump_folder + "\\after_increase_precision.svg";
                ov::pass::VisualizeTree(dump_file).run_on_model(model);
                std::cout << "!!!!!!!!!!!!!!! Graph dumped to: " << dump_file << std::endl;
                break;
            }
