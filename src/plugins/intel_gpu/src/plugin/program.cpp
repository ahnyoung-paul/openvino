...
std::shared_ptr<cldnn::program> Program::BuildProgram(const std::vector<std::shared_ptr<ngraph::Node>>& ops,
    ...
    // So, do not check allow_new_shape_infer from ops for inner program build
    if (innerProgram) {
    ...
    if (createTopologyOnly) {
        ...
        try {
            program = cldnn::program::build_program(m_engine, *m_topology, m_config, false, false, false, get_task_executor());
