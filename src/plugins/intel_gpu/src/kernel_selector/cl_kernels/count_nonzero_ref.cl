...
#if COUNT_NONZERO_PARITAL_SUM
KERNEL (count_nonzero_ref_parital_sum)(
    OPTIONAL_SHAPE_INFO_ARG
    const __global INPUT0_TYPE* input,
    volatile __global uint* tmp_buffer,
    volatile __global uint* buf_length)
{
    ...
    work_group_barrier(CLK_GLOBAL_MEM_FENCE | CLK_LOCAL_MEM_FENCE);
    if (ldim0 == 0 && ldim1 == 0 && ldim2 == 0) {
        tmp_buffer[group_id] = local_count;
    ...
    *buf_length = (uint)get_num_groups(2);
...
#else
KERNEL (count_nonzero_ref_final)(
    OPTIONAL_SHAPE_INFO_ARG
    volatile __global uint* tmp_buffer,
    volatile __global uint* buf_length,
...
{
    output[0] = 0;
    const uint buf_size = buf_length[0];
    for (uint gid = 0; gid < buf_size; gid++ ) {
        output[0] = output[0] + tmp_buffer[gid];
        // printf("nonzero_count_sum - tem_buffer[%3d / %3d](%3d),output[%3d]\n", gid, buf_size, tmp_buffer[gid], output[0]);
    }
}
#endif // COUNT_NONZERO_PARITAL_SUM
