...
| `KEY_GPU_PLUGIN_`<br>`PRIORITY` | `GPU_PLUGIN_PRIORITY_<HIGH\|LOW>`  <br/> `GPU_QUEUE_PRIORITY_<LOW\|HIGH\|MED\|DISABLED>`   <br/> `GPU_HOST_TASK_PRIORITY_<HIGH\|LOW\|ANY>`  | `GPU_QUEUE_PRIORITY_DISABLED` <br/>  `\|GPU_HOST_TASK_PRIORITY_ANY`             | Specifies two types of priority: host task priority and OpenCL queue priority.<br><br>Host task priority is specified by GPU_HOST_TASK_PRIORITY_[level] and there are three types of task levels: `HIGH`, `LOW`, and `ANY`. Note that `HIGH` and `LOW` have only effect when tbb is used for multithreading LoadNetwork and the host processor is hybrid processors. For hybrid processors, `HIGH` task priority level sets the core types to be used by tbb as BIG cores while `LOW` sets them as `LITTLE` cores. If the host processor is not hybrid core or the multi threading is not using tbb, it is set as `ANY`, which is the default type.<br><br>OpenCL queue priority is specified by `GPU_QUEUE_PRIORITY_[level]` and there are four types of levels: `HIGH`, `MED`, `LOW`, and `DISBLED`, where the default value is `DISABLED`. Before usage, make sure your OpenCL driver supports appropriate extension.<br><br>Basically `GPU_PLUGIN_PRIORITY` can be set as combination of the two priority types, such as<br>-`GPU_QUEUE_PRIORITY_HIGH\|GPU_HOST_TASK_PRIORITY_HIGH` or<br>-`GPU_QUEUE_PRIORITY_LOW\|GPU_HOST_TASK_PRIORITY_HIGH`.<br><br>Also it can be set as a more abstract level `PLUGIN_PRIORIY_[level]`, which represents combination of the two priorities as follows:<br>-`GPU_PLUGIN_PRIORITY_HIGH : GPU_QUEUE_PRIORITY_HIGH\|GPU_HOST_TASK_PRIORITY_HIGH`<br>-`GPU_PLUGIN_PRIORITY_LOW : GPU_QUEUE_PRIORITY_LOW\|GPU_HOST_TASK_PRIORITY_LOW`<br><br>The default of `KEY_GPU_PLUGIN_PRIORITY` is `GPU_QUEUE_PRIORITY_DISABLED\|GPU_HOST_TASK_PRIORITY_ANY`.<br>  |
